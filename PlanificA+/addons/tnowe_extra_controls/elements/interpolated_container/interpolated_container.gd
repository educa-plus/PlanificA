...
var _children_rotation_start = {}
var _children_rotation_end = {}
var _children_position_start = {}
...
func fit_interpolated(child : Control, rect : Rect2):
	...
	#print(child.get_global_transform())
	child.rotation = 0
	...
	_children_rotation_start[child_index] = rotation
	_children_position_start[child_index] = child.position
	fit_child_in_rect(child, rect)
	#_children_xforms_end[child_index] = Transform2D(Vector2(1, 0), Vector2(0, 1), child.position + child.size * 0.5)
	_children_xforms_end[child_index] = Transform2D(child_start_xform.x, child_start_xform.y, child.position + child.size * 0.5)
	_children_sizes_end[child_index] = child.size
	_children_rotation_end[child_index] = child.rotation 
...
func _process(delta : float):
	...
	for i in children.size():
		...
		#cur_child.rotation = _children_rotation_start[i]
		#print(_children_rotation_start[i])
		...
		#print(child_xform.get_rotation())
		#cur_child.rotation = _children_rotation_start[i]
		#else :
			#var child_xform = _children_xforms_start[i].interpolate_with(_children_xforms_end[i], progress_eased)
			#cur_child.size = _children_sizes_start[i].lerp(_children_sizes_end[i], progress_eased)
