...
@onready var sequence_options_menu_button = $EveryThing/Canva_Planif/BarreOption/Options/SequenceOption
@onready var export_file_dialog = $EveryThing/Canva_Planif/BarreOption/Options/Export/FileDialog
@onready var class_by_node = %ClassBy
...
func _ready():
	...
	class_by_node._update_save_path()
	...
	sequence_options_menu_button.connect("create_sequence", _choose_name.bind("new_sequence"))
	sequence_options_menu_button.connect("add_sequence", _pop_sequence_menu.bind(true))
...
func _on_file_dialog_file_selected(path: String):
	...
	if image:
		...
		var save_path = ""
		if not class_by_node.is_custom_path :
			save_path = current_path + "_" + title_label.text
		else :
			save_path = class_by_node.save_path
		path = path + ".png"
...
func _new_planif():
	...
	class_by_node._update_save_path()
	current_path = class_by_node.save_path
	if not class_by_node.is_custom_path :
		var path = current_path + "_" + title_label.text + ".tres"
		if FileAccess.file_exists(path) :
			current_user_path = path
	else :
		current_user_path = class_by_node.save_path
...
func save_instance_data_resource():
	...
	#var path = ""
	print(class_by_node.save_path)
	print(current_user_path)
	class_by_node._update_save_path()
	print(class_by_node.save_path)
	current_path = class_by_node.save_path
	if not class_by_node.is_custom_path :
		current_user_path = current_path + "_" + title_label.text + ".tres"
	else :
		current_user_path = class_by_node.save_path

	if class_by_node.items_data["Exporter"]["checked"] :
		var user_path_error = ResourceSaver.save(save_resource, current_user_path, ResourceSaver.FLAG_COMPRESS)
		if user_path_error == OK:
			print("Planification save in : ", current_user_path)
			print(global_variables.current_selected_group)
		else:
			print("Error during the save of : ", current_user_path, " Erreur: ", user_path_error)
...
func collect_save_data_into_resource():
	...
	save_resource.items_data = class_by_node.items_data
	...
	if class_by_node.is_custom_path :
		save_resource.is_custom_path = class_by_node.is_custom_path
		save_resource.custom_path = class_by_node.save_path
...
func apply_save_data_from_resource(save_resource: SaveCanvaData):
	...
	class_by_node.is_custom_path = loaded_is_custom_path
	if loaded_is_custom_path :
		class_by_node.save_path = loaded_custom_path
	...
	class_by_node.items_data = loaded_items_data
	class_by_node._setup_menu_items()
...
func _choose_name(name_function):
	...
	if name_function == "new_sequence" :
		choose_name_label.text = "Comment voulez-vous nommer cette séquence ?"
		choose_name_line_edit.connect("text_submitted", _create_new_sequence)
...
func _create_new_sequence(choosen_name):
	if not DirAccess.dir_exists_absolute("user://sequences/" + "sequence_" + choosen_name + "/") :
		choose_name_box.hide()
		var planif_resource = collect_save_data_into_resource()
		SequenceFunctions._create_new_sequence(choosen_name, planif_resource)
	else :
		print("this_name_already_exist")

func _pop_sequence_menu(is_sequence_selection):
	var sequence_menu_instance = sequence_menu.instantiate()
	sequence_menu_instance.is_sequence_selection = is_sequence_selection
	sequence_menu_instance.connect("selection_finished", _apply_sequence_menu_action)
	self.add_child(sequence_menu_instance)

func _apply_sequence_menu_action(sequence_selections):
	var planif_resource = collect_save_data_into_resource()
	for sequence_box in sequence_selections :
		SequenceFunctions._add_planif_to_sequence(sequence_box.sequence_name, planif_resource)
